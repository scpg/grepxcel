...
The most common pattern is: a label cell identifies a value cell nearby.

```
DEPARTMENT:     Accounting        ← B3 = label, C3 = value
CONTACT PHONE:  +1-555-0100       ← B4 = label, C4 = value
```
...
A *mini-table* is a repeating block of rows: one or more header rows (column titles),
one or more data rows (the actual records), and optionally a footer row (totals, notes).
...
|---|---|---|
| **Table name** | `items` | A short label used in the session log. ⚠️ See note below about JSON output. |
| **Full table range** | `B11:F29` | The bounding box of the *entire* block, including header and footer rows. |
...
| # | Issue | Impact |
|---|---|---|
| 1 | `-o pattern.xlsx` writes CSV text, not a real xlsx workbook | Confusing; file opening warns in Excel |
| 2 | Footer rows emit `HEADER:1` instead of `FOOTER:1` | Pattern may not match correctly |
| 3 | Table name is not used in the pattern | User expects nested JSON under the table name |
| 4 | Duplicate `lbl:` names (e.g. all named "empty") cause silent wrong matches | Pattern may match wrong cells |
| 5 | Wizard does not warn when two labels get the same name | User can create bad patterns unnoticed |
| 6 | Auto-suggested header column names not always useful (need clearer guidance) | User confusion about what to type |
