...
def fixture_01():
    ...
    for row in [
        ...
        ['def:', 'inv.number',   'string',   r'[A-Z]{2}[0-9]{6}'],
        ['def:', 'inv.date',     'date',     r'.*'],
        ['def:', 'inv.due_date', 'date',     r'.*'],
        ['def:', 'client.name',  'string',   r'.+'],
        ['def:', 'client.email', 'string',   r'.+@.+'],
        ['def:', 'amount.net',   'currency', r'.*'],
        ['def:', 'amount.vat',   'currency', r'.*'],
        ['def:', 'amount.gross', 'currency', r'.*'],
        ['START:'],
...
def fixture_02():
    ...
    for row in [
        ['config:', 'read.direction', 'LR'],
        ['def:', 'header.product', 'string',  'Product'],
        ['def:', 'header.sku',     'string',  'SKU'],
        ['def:', 'header.qty',     'string',  'Qty'],
        ['def:', 'header.price',   'string',  'Price'],
        ['def:', 'item.name',      'string',  r'.+'],
        ['def:', 'item.sku',       'string',  r'[A-Z]{3}[0-9]{3}'],
        ['def:', 'item.qty',       'integer', r'[1-9][0-9]*'],
        ['def:', 'item.price',     'currency', r'.*'],
        ['START:'],
        ['table:*'],
        [None, 'HEADER:1', 'header.product', 'header.sku', 'header.qty', 'header.price'],
...
def fixture_03():
    ...
    for row in [
        ['config:', 'read.direction', 'LR'],
        ['config:', 'currency.sign',  '€'],
        # cell fields (header block)
        ['def:', 'lbl.po',       'string',   'PO Number:'],
        ['def:', 'po.number',    'string',   r'PO-[0-9]{4}'],
        ['def:', 'lbl.date',     'string',   'Date:'],
        ['def:', 'po.date',      'date',     r'.*'],
        ['def:', 'lbl.vendor',   'string',   'Vendor:'],
        ['def:', 'vendor.name',  'string',   r'.+'],
        ['def:', 'lbl.ref',      'string',   'Ref:'],
        ['def:', 'vendor.ref',   'string',   r'[A-Z0-9]+'],
        # table column headers
        ['def:', 'col.item',     'string',   'Item'],
        ['def:', 'col.desc',     'string',   'Description'],
        ['def:', 'col.qty',      'string',   'Qty'],
        ['def:', 'col.price',    'string',   'Unit Price'],
        ['def:', 'col.total',    'string',   'Total'],
        # table data fields (.{3,50} → "X" in the data will fail → warning)
        ['def:', 'row.item',     'string',   r'.{3,50}'],
        ['def:', 'row.desc',     'string',   r'.*'],
        ['def:', 'row.qty',      'integer',  r'[1-9][0-9]*'],
        ['def:', 'row.price',    'currency', r'.*'],
        ['def:', 'row.total',    'currency', r'.*'],
        # footer
        ['def:', 'footer.label', 'string',   'Grand Total'],
        ['def:', 'footer.value', 'currency', r'.*'],
        ['START:'],
        ['cell:1', 'lbl.po'],
        ['cell:1', 'po.number'],
        ['cell:1', 'lbl.date'],
        ...
        ['cell:1', 'vendor.ref'],
        ['table:*'],
        [None, 'HEADER:1', 'col.item',     'col.desc',  'col.qty',  'col.price',  'col.total'],
